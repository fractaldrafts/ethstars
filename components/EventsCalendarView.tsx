...
export default function EventsCalendarView({ events, onEventClick }: EventsCalendarViewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Selected Date Events Panel - Sticky Sidebar Style */}
      {selectedDate && selectedDateEvents.length > 0 && (
        <div className="border border-[rgba(245,245,245,0.08)] rounded-lg p-4 bg-[rgba(245,245,245,0.04)] sticky top-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-500">({selectedDateEvents.length} event{selectedDateEvents.length !== 1 ? 's' : ''})</span>
            </div>
            <button
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h5 className="text-sm font-medium text-white group-hover:text-red-400 transition-colors truncate">
                          {event.title}
                        </h5>
                        <span className={`text-xs font-medium border px-2 py-0.5 rounded flex-shrink-0 ${config.bg} ${config.border} ${config.text}`}>
                          {formatLocationType(event.locationType)}
                        </span>
                      </div>
                      <p className="text-xs text-zinc-500 mb-2">{event.organizer}</p>
                      ...
                      {event.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {event.tags.slice(0, 3).map(tag => (
                            <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-[rgba(245,245,245,0.08)] text-zinc-500 rounded">
                              {tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <ArrowUpRight className="w-4 h-4 text-zinc-500 group-hover:text-red-400 transition-colors flex-shrink-0" />
                  </div>
                </a>
              )
            })}
